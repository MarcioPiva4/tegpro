...
export default function Assessment() {
  ...
  const [activeSlideIndex, setActiveSlideIndex] = useState<number | null>(null); // <-- null inicial
      ...
      <div className="w-full max-w-[1200px] h-[500px] relative">
        ...
        >
          {videos.map((video, i) => (
            <SwiperSlide key={i}>
              <div
                className="relative flex justify-end items-end h-full w-full group cursor-pointer"
                onClick={() => setActiveSlideIndex(i)}
              >
                {/* TODOS Iframes renderizados */}
                <div
                  className="absolute top-0 left-0 w-full h-full flex justify-center items-center rounded-2xl overflow-hidden transition-opacity duration-300"
                >
                  <iframe
                    width="370"
                    height="500"
                ...
                <div
                  className={`flex flex-col justify-center items-center bg-white rounded-2xl text-[var(--text-secondary)] pb-5 pt-7 px-4 absolute bottom-0 mx-5 mb-5 transition-all duration-300 z-20
                    ${
                      activeSlideIndex === i
                ...
                >
                  ...
                  <h2 className="text-[18px] font-semibold text-center">{video.nome}</h2>
                  <span className="text-[12px]">{video.cargo}</span>
                  <p className="text-[14px] my-4 text-center">
                    “Comecei do zero e em 2 meses fiz minha primeira venda. O curso é direto e prático!”
                  </p>
